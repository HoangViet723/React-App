...
const AddMembers = () => {
  ...
  const [users, setUsers] = useState([]); // Thêm state này để lưu trữ danh sách người dùng
...
// Chuyển đổi user.birthDate thành một đối tượng Date
  ...
  const handleSave = () => {
    ...
    axios
      .post(`http://localhost:3001/member/`, member)
      .then((response) => {
        console.log("Member added successfully:", response.data);
        navigate("/member"); // Chuyển hướng về "/member" sau khi thêm thành công
      })
      .catch((error) => {
